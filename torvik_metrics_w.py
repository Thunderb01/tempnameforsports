"""
torvik_metrics_w.py — Women's BTP metrics + NIL valuation pass
================================================================================================
Fork of torvik_metrics.py for the women's tables. Same compute formulas
(compute_cdi/dds/sei/ath/ris/nil_valuation), just pointed at the `w_*`
tables and defaulting to `data/trank_ncaaw_2026.csv`.

When you tune the women's scoring later (different weights, different NIL
spread, etc.), the formulas to change live in the same compute_* functions
below. The script structure stays parallel to men's so a fix on one side
is easy to mirror on the other.

Reads the women's Torvik CSV, matches players to Supabase by (name, team),
writes:
  - Raw Torvik inputs to w_player_stats (torvik_* columns)
  - Computed BTP metrics to w_player_stats (cdi, dds, sei, ath, ris)
  - NIL valuation to w_players (nil_valuation)
  - Birth year to w_players (birth_year)
...
def parse_args():
    ...
    p.add_argument("--csv",     default="data/trank_ncaaw_2026.csv")
...
def main():
    ...
    if not args.dry_run:
        ...
        while True:
            _res = _db_tmp.table("w_player_stats") \
        ...
        _pl_rows, _page = [], 0
        while True:
            _res = _db_tmp.table("w_players").select("id, name, current_team, height, weight") \
                          .range(_page * _ps, (_page + 1) * _ps - 1).execute()
    ...
    if not args.dry_run:
        ...
        while True:
            res = db.table("w_players").select("id, name, current_team, height, nil_valuation, torvik_pid") \
        ...
        _sr_rows, _page = [], 0
        while True:
            _sr = db.table("w_player_stats").select("id, player_id, calendar_year, cdi") \
    ...
    for _, row in df.iterrows():
        ...
        if not args.historical:
            ...
            if player_patch:
                db.table("w_players").update(player_patch).eq("id", player_id).execute()
                if nil is not None:
        ...
        if stats_id:
            db.table("w_player_stats").update(stats_patch).eq("id", stats_id).execute()
        else:
