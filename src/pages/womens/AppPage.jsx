...
// be kept in sync with the men's fork's and useRosterBoard's by hand.
...
function RosterStrengthPanel({ calc, onOpenModal, allPlayers = [], teamFreshmenAll = [], rosterIntl = [], freshmanTiers = [], userTeam = "", customOrder, setCustomOrder, starterCounts, setStarterCounts }) {
  ...
  function axisValues(p) {
    ...
    if (p._freshmanEffect != null) {
      ...
      return PROFILE_AXES.map(() => v);
    }
    return null;
  }

  const rosterProfile = (() => {
    let pool = scoringPool.filter(p => !isFreshmanP(p));
