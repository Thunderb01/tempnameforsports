...
// Roster-builder localStorage state shape (settings/roster/statusById/
// retentionById/nilById/removedIncomings/intlPlayers) is identical across
// sports and hasn't changed in this migration, so it stays one shared version.
const STORAGE_VERSION = 15; // bump this whenever the state shape changes

/**
 * Factory producing one fully independent roster-board hook per sport.
 * Was two 900-line byte-for-byte-identical files (useRosterBoard.js /
 * useWomensRosterBoard.js, differing only in table/storage-key names) —
 * merged here since CLAUDE.md's men's/women's fork exists for scoring
 * divergence, not for hooks with zero business-logic difference.
 *
 * Each call creates its own closure-scoped module caches (_boardCache,
 * _rosterCache, _incomingCache) so the men's and women's instances never
 * share state, exactly as the two separate files never did.
 */
export function createRosterBoardHook(cfg) {
  function loadSessionCache() {
    try {
      const raw = sessionStorage.getItem(cfg.sessionBoardKey);
      if (!raw) return [];
      const { players, ts, v } = JSON.parse(raw);
      if (v !== cfg.sessionBoardVer || Date.now() - ts > SESSION_BOARD_TTL) {
        sessionStorage.removeItem(cfg.sessionBoardKey);
        return [];
      }
      return players ?? [];
    } catch { return []; }
  ...
  function saveSessionCache(players) {
    try { sessionStorage.setItem(cfg.sessionBoardKey, JSON.stringify({ players, ts: Date.now(), v: cfg.sessionBoardVer })); } catch {}
  }
  ...
  function useBoard(team, userId) {
    ...
    const loadPortalBoard = useCallback(async () => {
      ...
      // Reassign committed transfers to their destination team for scoring
      // purposes. Without this, a player committed to Texas but still shown
      // under his old school counts toward the old school's roster strength,
      // not Texas's — so every team's static score systematically misses
      // their incoming class. player_status/transfer_status/transfer_to_team
    ...
    const loadReturningRoster = useCallback(async (teamName) => {
      ...
      const returningIdSet = new Set(returning.map(p => p.id));

      // Incoming transfers: players committed to this team, not already on
      // it. transfer_status/transfer_to_team now live inline on
      // vw_players/vw_w_players, so one query gets both the id list AND full
      // player data — previously this was two separate round-trips (ids from
      // portal_transfers, then a join back to vw_players for full data).
      let incomingMapped = [];
      if (_incomingCache[teamName]) {
        ...
        incomingMapped = (incomingFull || [])
          .filter(row => !returningIdSet.has(row.id))
          .map(row => ({
            id:             row.id,
            name:           row.name,
            team:           row.current_team,
      ...
      try {
        const { data: intlCommitted } = await supabase
          .from(cfg.internationalPlayersTable)
          .select("*")
      ...
      // Map player_id → portal retention bucket, derived from the inline
      // transfer_status already present on each returning player's row:
      ...
      // Admin-set player_status → retention value mapping (also inline now)
      const adminStatusMap = {};
      returning.forEach(p => {
        const mapped = PLAYER_STATUS_TO_RETENTION[p.player_status];
        if (mapped) adminStatusMap[p.id] = mapped;
      });
    ...
    useEffect(() => {
      supabase.from(cfg.freshmanTiersTable).select("name, effect, color, sort").order("sort")
    ...
    useEffect(() => {
      supabase.from(cfg.teamFreshmenTable).select("id, team, name, pos, tier, recruiting_class, sei, ath, ris, dds, cdi, nil_valuation")
    ...
    const loadCustomPlayers = useCallback(async (teamName, uid) => {
      ...
      const { data, error } = await supabase
        .from(cfg.customRosterPlayersTable)
        .select("*")
        .eq("team", teamName)
        .eq("user_id", uid)
        .order("created_at");
    ...
    async function removeCustomPlayer(id) {
      const { error } = await supabase.from(cfg.customRosterPlayersTable).delete().eq("id", id);
    ...
    async function persistCustomPlayerNil(id, nil_offer) {
      const val = Math.max(0, Number(nil_offer) || 0);
      const { error } = await supabase.from(cfg.customRosterPlayersTable).update({ nil_offer: val }).eq("id", id);
    ...
    const calc = useMemo(() => {
      ...
      // LEAVING_RETENTION_STATUSES (shared with AppPage's CMP_LEAVING_STATUSES via
      // src/lib/playerStatus.js) covers both raw player_status values and their
      // retention-mapped forms, keeping the user's pool and the every-team
      // baseline filtering on equivalent sets.
      const activeRosterReturners = returningPlayers.filter(p => !LEAVING_RETENTION_STATUSES.has(retentionById[p.id] || "returning"));
...
const MENS_CONFIG = {
  vwPlayersTable:             "vw_players",
  teamPlayersTable:           "team_players",
  internationalPlayersTable:  "international_players",
  freshmanTiersTable:         "freshman_tiers",
  teamFreshmenTable:          "team_freshmen",
  customRosterPlayersTable:   "custom_roster_players",
